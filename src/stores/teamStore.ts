...
export const useTeamStore = create<TeamState>((set, get) => ({
  ...
  // Workflow actions (Legacy - for backward compatibility)
  addAgentToWorkflow: (agentId, position) =>
    set((state) => {
      console.log('[teamStore] addAgentToWorkflow called:', agentId, position);
      if (!state.currentTeam || !state.currentTeam.workflow) {
        console.log('[teamStore] No currentTeam or workflow, returning');
        return state;
      }
  ...
  removeAgentFromWorkflow: (agentId) =>
    set((state) => {
      if (!state.currentTeam || !state.currentTeam.workflow) return state;
  ...
  getWorkflowAgents: () => {
    ...
    if (!currentTeam || !currentTeam.workflow) return [];
